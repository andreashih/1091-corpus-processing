...
### 2. 實作 Frequency Distribution Analysis
...
### 3. 實作 Lexical Coverage Estimation
...
首先，我們要計算 lexical of seen types。也就是說，我們想要知道語料中至少出現過兩次的單詞總共有幾個。計算方式是整個 vocabulary size 減掉其中只出現過一次的詞的數量。
...
這樣我們就可以計算八卦板與 Dcard 語料中 OOV 所佔的比例，大約分別是 0.545 與 0.591。
...
如果我們再進一步看 *hapax legomena* (只出現一次的詞) 在整個語料中的佔比，
...
就可以發現，我們剛才所計算的 `gossip_Vseen` 和 `dcard_Vseen` cover 到各 92% 的 token。剩下的 8% 就屬於 OOV。
...
以八卦板語料為例，假設我們用 1、10，和 1 億個 tokens 進行計算，expected OOV 分別會是：
...
```{r}
#EV(fzm_g, c(1e6, 10e6, 100e6))
#EV(fzm_d, c(1e6, 10e6, 100e6))
```
